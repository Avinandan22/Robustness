...
We have that <LaTeX formula='\mathbb{E}_{\theta \sim \mu}[\Pi(\theta^\prime|\theta)]=\mu(\theta^\prime)' inline={true}/> since <LaTeX formula='\mu' inline={true}/> is stationary. Thus, we can add and subtract the expectation of an arbitrarily chosen function <LaTeX formula='\lambda' inline={true}/> from the objective to obtain
