...
## The Problem Setup
We assume that our learning algorithm is trying to estimate a set of parameters <LaTeX formula='\theta' inline={true}/> (like the parameters of a neural network). We study online learning algorithms of the form <LaTeX formula='\theta^\prime \gets F(\theta, z)'/> where  <LaTeX formula='z' inline={true}/> denotes the current datapoint or batch of datapoints.
