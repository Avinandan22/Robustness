...
<LaTeX formula='\Pi_{z_{\text{adv}}}(\theta^\prime|\theta)= \epsilon \delta(F(\theta, z))\text{ where } z \sim \epsilon\delta(z^{\text{adv}}) + (1-\epsilon)\mathbb{P}_{\text{data}}' />

The adversary's goal is to maximize
...
## Derivation of the Compute Certificate
