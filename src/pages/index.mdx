...
<div className="border-l-4 border-blue-500 bg-gray-900 p-4 text-white">
  <strong>Theorem:</strong> For any function <LaTeX formula='\lambda(\theta)' inline={true}/>, the worst case objective achieved by the adversary is at most
...
We have that <LaTeX formula='\mathbb{E}_{\theta \sim \mu}[\Pi(\theta^\prime|\theta)]=\mu(\theta^\prime)' inline={true}/> since <LaTeX formula='\mu' inline={true}/> is stationary. Thus, we can add and subtract the expectation of an arbitrarily chosen function <LaTeX formula='\mu' inline={true}/> from the objective to obtain
...
<LaTeX formula='\sup_{\theta, z_{\text{adv}}}[\ell_{\text{adv}}(\theta)-\lambda(\theta) + \mathbb{E}_{\theta^\prime \sim \Pi_{z_{\text{adv}}}(\theta^\prime|\theta)}[\mu(\theta^\prime)]]'/>
...
where first component focuses on the learning algorithm's ability to perform effectively without an adversary and the second component provides an upper bound on the potential worst-case loss the algorithm might face when an adversary is involved. The key here is to find a defense that works well on average, even when faced with new, unseen data distributions. We find that this approach can be applied successfully to mean estimation, binary and multiclass classification and learning reward functions from human feedback.
...
We conduct reward learning experiments using Helpsteer, an open-source helpfulness dataset that is used to align large language models to become more helpful, factually correct and coherent, while being adjustable in terms of the complexity and verbosity of its responses. 
...
Based on our results, our our meta-learning setup allows us to learn hyperparameters of the learning algorithm which are more robust that arbitrarily chosen hyperparameters.
