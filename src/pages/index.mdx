...
We have that <LaTeX formula='\mathbb{E}_{\theta \sim \mu}[\Pi(\theta_^\prime|\theta)]=\mu(\theta^\prime)' inline={true}/> since <LaTeX formula='\mu' inline={true}/> is stationary (note that we drop the subscript <LaTeX formula='\Pi_{z_{\text{adv}}}'/> for brevity). Thus, we can add and subtract the expectation of an arbitrarily chosen function <LaTeX formula='\mu' inline={true}/> from the objective to obtain
