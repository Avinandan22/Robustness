...
## The Problem Setup
We assume that our learning algorithm is trying to estimate a set of parameters <LaTeX formula='\theta' inline={true}/> (like the parameters of a neural network). We study online learning algorithms of the form <LaTeX formula='\theta^\prime \gets F(\theta, z)' inline={true}/> where  $z$ denotes the current datapoint or batch of datapoints.

Some of the data points may be poisoned by an adversary who wants to mislead the learning algorithm towards parameters that optimize some adversarial objective <LaTeX formula='\ell_{\text{adv}}(\theta)' inline={true}/>, which could represent the prediction error of the model (for an adversary simply targeting reducing the performance of the learning algorithm) or more targeted objectives (like the model having a favorable view of a certain political party). At each step, the algorithm might receive poisoned data with a certain probability. This is controlled by a parameter <LaTeX formula='\epsilon' inline={true}/>, which determines what fraction of data points are poisoned.
